(function () {
  ...
  function bindNavigation() {
    ...
    $('#newAlbumBtn').addEventListener('click', () => showView('basic'));
  }
  ...
  function createManualSlide(overrides = {}) {
    ...
    return {
      ...
      frameArchive,
      frames: normalizeFrames(frameArchive, photoCount, slideId),
      manualText: {
        title: '',
        subtitle: '',
        message: '',
  ...
  function normalizeFrames(frames, photoCount, slideId) {
  ...
  function renderSlideEditForm() {
    ...
    form.innerHTML = `
      ...
      <div class="manual-note">空欄の項目は自動生成文を表示します。手動編集文を入れると、その内容がプレビューとPDFに反映されます。</div>
      <div class="editor-grid">
  ...
  function renderSlideEditPreview() {
    ...
    const slide = state.slides.find((item) => item.slideId === state.currentSlideEditId);
    preview.innerHTML = '';
    if (slide) preview.appendChild(window.SlideBuilder.renderSlide(slide));
    const manualSlide = getCurrentManualSlide();
  ...
  function frameEditorMarkup(manualSlide) {
    ...
    if (!manualSlide.frames.length) {
      return '<div class="frame-editor"><h3>フレーム</h3><p>このスライドは写真0枚の言葉中心レイアウトです。</p></div>';
    }
    return `
      <div class="frame-editor">
        <h3>フレーム配置</h3>
        <p class="save-hint">各フレームに写真を選択、または画像ファイルをドラッグ＆ドロップできます。写真はブラウザ内でCanvasリサイズされ、外部送信されません。</p>
  ...
  function frameRowMarkup(frame, index) {
    ...
    return `
      <div class="frame-row" data-frame-index="${index}">
        ...
        <label class="file-button secondary">写真追加<input data-frame-file="${index}" type="file" accept="image/jpeg,image/jpg,image/png,image/webp"></label>
    ...
    $$('#slideEditForm [data-manual-field]').forEach((field) => {
      field.addEventListener('change', () => {
        if (field.dataset.manualField === 'photoCount') {
          ...
          manualSlide.frames = normalizeFrames(manualSlide.frameArchive, manualSlide.photoCount, manualSlide.slideId);
  ...
  function duplicateCurrentManualSlide() {
    ...
    copy.frames = normalizeFrames(copy.frameArchive, copy.photoCount, copy.slideId);
  ...
  function serializeManualSlide(slide) {
    return {
      ...
      frameArchive: slide.frameArchive || slide.frames,
  ...
  function loadState(project) {
    ...
    state.creationMode = project.creationMode || 'auto';
  ...
  function normalizeManualSlide(slide) {
    ...
    return createManualSlide({
      ...slide,
      photoCount,
      frameArchive: normalizeFrameArchive(slide.frameArchive || slide.frames || slide.frameData || [], slide.slideId),
      frames: normalizeFrames(slide.frames || slide.frameData || slide.frameArchive || [], photoCount, slide.slideId),
      manualText: slide.manualText || {},
